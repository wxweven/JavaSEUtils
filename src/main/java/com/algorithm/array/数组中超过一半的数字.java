 ...
 *
 * 思路1：先用快排把数组排好序，然后中间的那个数就是出现次数最多的，快排复杂度为O(nlogn)。
 * 但实际中，我们不用一次性完成快排，只需快排一轮后检查我们选择的快排的key所在的下标是不是数组中间数就行了。
 * 总复杂度O(n)，这种方法会修改数组，因此要看是否允许这样做，快排是不稳定的排序。
 *
 * 思路2：复杂度也是O(n)且不需修改数组，方法是根据数组特点：定义一个记录出现次数的变量并初始为1，然后从头开始遍历，
